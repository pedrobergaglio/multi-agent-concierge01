...
class AuthenticateAgent(Workflow):
    #"""A dummy workflow with only one step sending back the input given."""

    @step()
    async def authenticate(self, ev: StartEvent) -> StopEvent: #Events.ConciergeEvent:
        ctx = get_context()
...
authenticate_workflow = AuthenticateAgent()


async def main():
    print("Running authenticate_workflow")
    print(await authenticate_workflow.run())
